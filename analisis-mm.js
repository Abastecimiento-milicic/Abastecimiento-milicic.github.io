(function() {
  ...
  function downloadExternalFilteredCSV(filename) {
    ...
    fetchWithCache(filename + "?t=" + window.CACHE_BUSTER)
      .then(async text => {
        ...
        const filteredRowsArray = m.filter((row, idx) => {
        ...
        const excelFilename = filename.replace(".csv", "") + "_" + label + ".xlsx";
        await window.saveAsExcel(excelFilename, "Datos", fileHeaders, filteredRowsArray);
      })
      .catch(err => {
        console.error(err);
  ...
  function buildValorizacionStock(rows) {
    ...
    let total = 0;
    rows.forEach(r => {
      const rub = clean(r[colRubro]) || "SIN RUBRO";
      const val = toNumber(r[colValor]);
      map[rub] = (map[rub] || 0) + val;
      total += val;
    });

    const items = Object.entries(map).map(([rubro, val]) => ({ rubro, val }));
    items.sort((a, b) => b.val - a.val);

    let cumSum = 0;
    items.forEach(it => {
      cumSum += it.val;
      const p = total > 0 ? (it.val / total * 100) : 0;
      const cumPct = total > 0 ? (cumSum / total * 100) : 0;
      
      const tr = document.createElement("tr");
      tr.innerHTML = `
        <td style="font-weight: 500; color: #1e293b; text-align: left;">${it.rubro}</td>
        <td style="text-align: right;">$ ${fmtMoney(it.val)}</td>
        <td style="text-align: right;">${p.toFixed(1).replace(".", ",")}%</td>
        <td style="text-align: right;">${cumPct.toFixed(1).replace(".", ",")}%</td>
      `;
  ...
  async function downloadStaticFile(filename) {
    try {
      const text = await fetchWithCache(filename + "?t=" + window.CACHE_BUSTER);
      const m = parseDelimited(text, DELIM);
      if (m.length < 1) {
        showError("El archivo " + filename + " está vacío.");
        return;
      ...
      const excelName = filename.replace(".csv", ".xlsx");
      await window.saveAsExcel(excelName, "Datos", fileHeaders, rows);
    } catch (err) {
      console.error(err);
      showError("Hubo un problema descargando " + filename);
    }
  }

  async function downloadFilteredBaseMM() {
    ...
    const filename = `ANALISIS-MM_${label}.xlsx`;

    await window.saveAsExcel(filename, "Analisis MM", headers, rows, headers);
  }

  async function downloadSinRubro() {
    if (!headers || !headers.length || !data || !data.length) {
      showError("Todavía no se cargó el archivo ANALISIS-MM.csv.");
      return;
    }
    ...
    await window.saveAsExcel("SIN_RUBRO.xlsx", "Sin Rubro", headers, out, headers);
  }

  async function downloadByEstadoValue(estadoExacto, filename) {
    if (!headers || !headers.length || !data || !data.length) {
      showError("Todavía no se cargó el archivo ANALISIS-MM.csv.");
      return;
    }
    ...
    await window.saveAsExcel(excelFilename, estadoExacto.substring(0, 31), headers, out, headers);
  }
  ...
  window.initMM = function() {
    ...
    document.getElementById("mm_btnDLStockBase374")?.addEventListener("click", () => downloadStaticFile("Stock Base Obra 0374.csv"));
    document.getElementById("btnDLStockBase374")?.addEventListener("click", () => downloadStaticFile("Stock Base Obra 0374.csv"));

    document.getElementById("mm_btnDLStockBase378")?.addEventListener("click", () => downloadStaticFile("Stock Base Obra 0378.csv"));
    document.getElementById("btnDLStockBase378")?.addEventListener("click", () => downloadStaticFile("Stock Base Obra 0378.csv"));
    ...
    const cacheKey = "processed_analisismm_" + window.CACHE_BUSTER;
    ...
    if (window.getCachedData) {
      window.getCachedData(cacheKey).then(cached => {
        if (cached && cached.headers && cached.data) {
          ...
          console.log("[cache-utils] Loaded preprocessed analisis-mm data from cache.");
        } else {
          loadFromCsv();
        }
      }).catch(err => {
