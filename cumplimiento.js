...
function pickHeaderByIncludes(headers, needles) {
  const up = headers.map(h => clean(h).toUpperCase());
  for (const n of needles) {
    const nu = clean(n).toUpperCase();
    const idx = up.findIndex(h => h === nu || h.includes(nu));
...
window.addEventListener("DOMContentLoaded", () => {
  ...
  fetch(csvUrl)
    ...
    .then(text => {
      ...
      CLIENT_COL = pickHeaderByIncludes(headers, CLIENT_CANDIDATES);
      if (!CLIENT_COL) {
      ...
      CLASIF2_COL = pickHeaderByIncludes(headers, CLASIF2_CANDIDATES) || null;
      GCOC_COL = pickHeaderByIncludes(headers, GCOC_CANDIDATES) || null;
