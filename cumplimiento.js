...
function buildChartMes(rows) {
  ...
  for (const r of rows) {
    ...
    const dv = toNumber(r[DEMORA_COL]); // incluye negativos
  ...
  const pAT = months.map((_,i)=> tot[i] ? (qAT[i]/tot[i])*100 : 0);
  const pFT = months.map((_,i)=> tot[i] ? (qFT[i]/tot[i])*100 : 0);
  const pNO = months.map((_,i)=> tot[i] ? (qNO[i]/tot[i])*100 : 0);
  ...
  const traces = [
    {
      ...
      x: months, y: pAT,
      customdata: qAT,
      hovertemplate: "%{x}<br>%{fullData.name}: %{customdata:,d} (%{y:.1f}%)<extra></extra>",
    ...
    },
    {
      ...
      x: months, y: avgD,
      yaxis: "y2",
      hovertemplate: "%{x}<br>%{fullData.name}: %{y:.0f} días<extra></extra>",
      line: { width: 3, color: COLORS.blue },
      marker: { size: 7, color: COLORS.blue }
    }
  ];

  const layout = {
    margin: { l: 55, r: 60, t: 6, b: 55 },
    barmode: "stack",
    bargap: 0.25,
