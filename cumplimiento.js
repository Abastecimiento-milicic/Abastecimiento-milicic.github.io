(function() {
  ...
  function filteredRowsNoMes() {
    ...
    const centros = getSelValues("cumpl_centroSelect");
    if (centros.length && CENTRO_COL) rows = rows.filter(r => centros.includes(clean(r[CENTRO_COL])));
    return rows;
  }
  ...
  function renderCentros() {
    if (!CENTRO_COL) return;
    const centros = uniqSorted(data.map(r => r[CENTRO_COL]));
    fillSelect("cumpl_centroSelect", centros, "Todos");
  }
  ...
  window.initCumplimiento = function() {
    ...
    fetchWithCache(csvUrl + "?t=" + window.CACHE_BUSTER)
      .then(text => {
        ...
        setText("cumpl_centroHint", CENTRO_COL ? `Columna: ${CENTRO_COL}` : "Columna: (no encontrada)");
        ...
        document.getElementById("cumpl_centroSelect")?.addEventListener("change", (e) => {
          enforceAllOption(e.target);
          applyAll();
        });
        ...
        document.getElementById("cumpl_btnDownloadNO")?.addEventListener("click", () => {
          ...
          const centro = safeFilePart(selLabel("cumpl_centroSelect"));
