...
/* ============================
   CONFIG
============================ */
const CSV_CANDIDATES = ["CUMPLIMIENTO_2025.csv","CUMPLIMIENTO 2025.csv","CUMPLIMIENTO.csv","cumplimiento.csv"];
const DELIM = ";";

const FECHA_COL = "FECHA ENTREGA ESPERADA";
const FECHA_CANDIDATES = ["FECHA ENTREGA ESPERADA","MES ENTREGA","MES DE ENTREGA","FECHA ENTREGA","FECHA ENTREGA OC","MES ENTREGA ESPERADA"];
const DEMORA_COL = "DIAS DE DEMORA";
const DEMORA_CANDIDATES = ["DIAS DE DEMORA","DÍAS DE DEMORA","DIAS DEMORA","DIAS_DE_DEMORA"];

function avgDelay(rows){
  let s = 0, c = 0;
  for (const r of rows){
    const v = toNumAny(r[DEMORA_COL_ACT || DEMORA_COL]);
    if (!isNaN(v)){ s += v; c++; }
  }
  return c ? (s / c) : NaN;
}
...
let CLIENT_COL = null;
let FECHA_COL_ACT = null;
let DEMORA_COL_ACT = null;
...
function detectHeader(hdrs, candidates){
  const map = new Map();
  for(const h of hdrs){
    map.set(clean(h), h);
  }
  for(const c of candidates){
    const hit = map.get(clean(c));
    if(hit) return hit;
  }
  return null;
}
...
function getMonthKeyFromRow(r) {
  const d = parseDateAny(r[FECHA_COL_ACT || FECHA_COL]);
...
function fillSelect(selectId, values, placeholder = "Todos") {
  ...
  // Si hay TomSelect activo, actualizo opciones sin romper la UI
  const ts = sel.tomselect || null;
  const prevMulti = ts ? (Array.isArray(ts.getValue()) ? ts.getValue() : (ts.getValue() ? [ts.getValue()] : [])) : null;
  const prevSingle = !ts ? (sel.value || "") : null;

  // Reconstruyo opciones del <select>
  sel.innerHTML = "";
  ...
  // Rehidrato TomSelect si existe
  if (ts) {
    const allowed = new Set(values.map(clean));
    ts.clearOptions();
    ts.addOption(values.map(v => ({ value: v, text: v })));
    ts.refreshOptions(false);

    const keep = (prevMulti || []).filter(v => allowed.has(clean(v)));
    ts.setValue(keep, true);
  } else {
    // restauro valor previo si sigue existiendo
    if (prevSingle && values.includes(prevSingle)) sel.value = prevSingle;
  }
}
...
function rowsByClienteBase() {
  const cs = getSelMultiTom(tsCliente);
  if (!cs.length) return data;
  const set = new Set(cs.map(clean));
...
function filteredRowsNoMes() {
  ...
  const c2s = getSelMultiTom(tsClasif2);
  if (c2s.length && CLASIF2_COL) {
    const set = new Set(c2s.map(clean));
    rows = rows.filter(r => set.has(clean(r[CLASIF2_COL])));
  }

  const gcs = getSelMultiTom(tsGcoc);
  if (gcs.length && GCOC_COL) {
    const set = new Set(gcs.map(clean));
    rows = rows.filter(r => set.has(clean(r[GCOC_COL])));
  }
...
function filteredRowsByAll() {
  ...
  const meses = tsMes ? getSelMultiTom(tsMes) : (getSel("mesSelect") ? [getSel("mesSelect")] : []);
  if (!meses.length) return rows;
  const set = new Set(meses.map(clean));
  return rows.filter(r => set.has(clean(getMonthKeyFromRow(r))));
}
...
function updateKPIsMonthly(rows, months) {
  const selMeses = tsMes ? getSelMultiTom(tsMes) : (getSel("mesSelect") ? [getSel("mesSelect")] : []);
  const mes = selMeses.length ? selMeses[selMeses.length - 1] : "";
  if (!mes) return;
...
function buildChartMes(rows) {
  ...
  for (const r of rows) {
    ...
    const rawDem = clean(r[DEMORA_COL_ACT || DEMORA_COL]);
    if (rawDem !== "") {
...
function applyAll() {
  ...
  const baseParaGc = (() => {
    let r = baseCliente;
    const c2s = getSelMultiTom(tsClasif2);
    if (c2s.length && CLASIF2_COL) { const set = new Set(c2s.map(clean)); r = r.filter(x => set.has(clean(x[CLASIF2_COL]))); }
    return r;
  })();
...
window.addEventListener("DOMContentLoaded", () => {
  ...
  const tryLoad = (i) => {
    const csvUrl = CSV_CANDIDATES[i];
    if (!csvUrl) {
      showError("Error cargando CSV. Revisá el nombre del archivo y que esté en la raíz del repo. Probé: " + CSV_CANDIDATES.join(", "));
      return;
    }
    fetch(csvUrl)
      .then(r => {
      ...
      .then(text => {
        ...
        data = rows.slice(1).map(arr => {
          const o = {};
          for (let i = 0; i < headers.length; i++) {
            o[headers[i]] = (arr[i] ?? "").toString();
          }
          return o;
        }).filter(r => Object.values(r).some(v => String(v).trim() !== ""));

        CLIENT_COL = detectHeader(headers, CLIENT_CANDIDATES);
        CLASIF2_COL = detectHeader(headers, CLASIF2_CANDIDATES);
        GCOC_COL = detectHeader(headers, GCOC_CANDIDATES);
        FECHA_COL_ACT = detectHeader(headers, FECHA_CANDIDATES) || FECHA_COL;
        DEMORA_COL_ACT = detectHeader(headers, DEMORA_CANDIDATES) || DEMORA_COL;

        if (!CLIENT_COL) {
          showError("No encontré la columna de CLIENTE. Revisá el CSV. Headers: " + headers.join(" | "));
          return;
        }
// Inicializo TomSelect apenas tengo los selects (aunque no haya opciones todavía)
        initTomSelects();
      ...
      .catch(err => {
        // intento con el próximo nombre
        tryLoad(i + 1);
      });
  };

  // Inicializo TomSelect ya (se verán vacíos hasta cargar datos)
  initTomSelects();
  tryLoad(0);
});
