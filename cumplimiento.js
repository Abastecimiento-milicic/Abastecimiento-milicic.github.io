...
const GCOC_CANDIDATES = ["GRUPO DE CCOMPRA OC", "GRUPO DE COMPRA_OC"];
...
function fillSelect(selectId, values, placeholder = "Todos") {
  ...
  const prevMulti = isMulti ? getSelMulti(selectId) : [];
  const prev = isMulti ? "" : sel.value;

  sel.innerHTML = "";

  // En multi-select NO agregamos opción "Todos" (si no seleccionás nada = todos)
  if (!isMulti) {
  ...
  for (const v of values) {
    ...
    o.value = v;
    o.textContent = v;
    if (isMulti && prevMulti.includes(v)) o.selected = true;
    sel.appendChild(o);
  }
...
function selectAll(id, on=true){
  const sel = document.getElementById(id);
  if (!sel) return;
  for (const opt of sel.options){
    if (!opt.value) continue; // evita "Todos" si existiera
    opt.selected = on;
  }
}
...
window.addEventListener("DOMContentLoaded", () => {
  ...
  fetch(csvUrl)
    ...
    .then(text => {
      ...
      // Botones Todo / Ninguno (multiselección)
      document.addEventListener("click", (e) => {
        const btn = e.target.closest("[data-sel][data-act]");
        if (!btn) return;
        const id = btn.dataset.sel;
        const act = btn.dataset.act;
        if (act === "all") selectAll(id, true);
        if (act === "none") selectAll(id, false);
        applyAll();
      });
...
document.getElementById("clienteSelect")?.addEventListener("change", () => {
        ...
        if (c2) clearSelection("clasif2Select");
        const gc = document.getElementById("gcocSelect");
        if (gc) clearSelection("gcocSelect");
        applyAll();
      });
