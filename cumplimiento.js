...
// Estado de multiselección (dropdowns)
const multiState = {
...
function renderMultiLabel(id, values=null){
  const btnLabel = document.querySelector(`.multi-btn[data-sel="${id}"] .multi-label`);
  if (!btnLabel) return;
  const st = multiState[id];
  const arr = st ? Array.from(st) : [];
  if (!arr.length) { btnLabel.textContent = "Todos"; return; }
  if (values && arr.length === values.length) { btnLabel.textContent = "Todo"; return; }
  if (arr.length === 1) { btnLabel.textContent = arr[0]; return; }
  btnLabel.textContent = `${arr.length} seleccionados`;
}
...
function renderClientes() {
  ...
  (document.querySelector(`.multi[data-sel="clienteSelect"]`) ? fillMulti("clienteSelect", clientes, "Todos");
}
...
window.addEventListener("DOMContentLoaded", () => {
  // Multiselect dropdown: abrir/cerrar panel
  ...
  document.addEventListener("click", (e) => {
    ...
    // botones Todo / Ninguno
    const actBtn = e.target.closest("[data-sel][data-act]");
    if (actBtn){
      const id = actBtn.dataset.sel;
      const act = actBtn.dataset.act;
      // valores actuales del listado
      const values = Array.from(document.querySelectorAll(`.multi-list[data-sel="${id}"] input[type="checkbox"]`))
        .map(x => x.value);
      if (act === "all") selectAll(id, values, true);
      if (act === "none") selectAll(id, values, false);
      // reflejar checks
  ...
  document.addEventListener("change", (e) => {
    ...
    if (!chk) return;
    const list = chk.closest(".multi-list");
    const id = list.dataset.sel;
    const st = multiState[id] || (multiState[id] = new Set());
    if (chk.checked) st.add(chk.value);
    else st.delete(chk.value);
    // actualizar label según lista actual
    const values = Array.from(list.querySelectorAll('input[type="checkbox"]')).map(x => x.value);
    renderMultiLabel(id, values);
    applyAll();
  });
