  ...
  fetch(csvUrl)
    ...
    .then(text => {
      ...
      document.getElementById("btnClearFilters")?.addEventListener("click", () => {
        // Limpia TomSelect (multi)
        if (tsCliente) tsCliente.clear(true);
        if (tsClasif2) tsClasif2.clear(true);
        if (tsGcoc) tsGcoc.clear(true);
        if (tsMes) tsMes.clear(true);

        // Fallback si no hay TomSelect
        ["clienteSelect","clasif2Select","gcocSelect","mesSelect"].forEach(id => {
          const el = document.getElementById(id);
          if (el && !el.tomselect) el.value = "";
        });
