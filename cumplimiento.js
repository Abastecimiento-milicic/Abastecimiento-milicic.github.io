  ...
  chartMes = new Chart(canvas.getContext("2d"), {
    ...
    data: {
      ...
      datasets: [
        ...
        { type: "line", label: "Promedio dias de demora", data: avgD, yAxisID: "y1", borderColor: COLORS.blue, backgroundColor: COLORS.blue, pointRadius: 4, pointHoverRadius: 6, borderWidth: 3, tension: 0.25, fill: false, spanGaps: true, order: 100, datalabels: { display: false } },
      ]
    },
    options: {
      responsive: true,
      ...
      plugins: {
        ...
        tooltip: {
          callbacks: {
            label: (c) => {
              if (c.dataset?.yAxisID === "y1" || c.dataset?.type === "line") {
                const v = (c.parsed.y ?? 0);
                return ` ${c.dataset.label}: ${Math.round(Number(v))} días`;
              }
