...
function buildChartMes(rows) {
  // Si no me pasan filas, uso las filas filtradas actuales
  const inRows = Array.isArray(rows) ? rows : filteredRowsNoMes();

  // Preferimos D3 si está disponible (para dual-axis + línea siempre arriba)
  if (window.d3) {
    const agg = new Map();
    ...
    for (const r of inRows) {
      ...
      const dv = toNumber(r[DEMORA_COL]);
      if (Number.isFinite(dv)) { c.dsum += dv; c.dcnt += 1; }
    }

    const months = [...monthsSet].sort();
    const dataMes = months.map(m => {
      const o = agg.get(m) || { at:0, ft:0, no:0, dsum:0, dcnt:0 };
      const total = o.at + o.ft + o.no;
      const avgD = o.dcnt ? (o.dsum / o.dcnt) : null;
      return {
        m,
        at: o.at, ft: o.ft, no: o.no,
        pAT: total ? (o.at/total)*100 : 0,
        pFT: total ? (o.ft/total)*100 : 0,
        pNO: total ? (o.no/total)*100 : 0,
        avgD
      };
    });
    ...
    // contenedor: el .chart-wrap que contiene al canvas
    const wrap = canvas.parentElement;
    if (!wrap) return;

    // oculto canvas para evitar solapamientos
    canvas.style.display = "none";

    // limpio svg anterior
    wrap.querySelectorAll("svg.chart-mes-d3").forEach(el => el.remove());

    const d3 = window.d3;
    const w = wrap.clientWidth || 900;
    const h = 360;
    const margin = { top: 16, right: 52, bottom: 36, left: 42 };

    const svg = d3.select(wrap)
      .append("svg")
      .attr("class", "chart-mes-d3")
      .attr("width", "100%")
      .attr("height", h)
    ...
    // eje derecho para demoras
    const demoraVals = dataMes.map(d => d.avgD).filter(v => Number.isFinite(v));
    const dmin = demoraVals.length ? d3.min(demoraVals) : 0;
    const dmax = demoraVals.length ? d3.max(demoraVals) : 1;
    ...
    svg.append("g")
      ...
      .call(d3.axisLeft(y).ticks(10).tickSize(-(w - margin.left - margin.right)).tickFormat(""));
    ...
    const series = [
      { key: "pAT", color: colors.at },
      { key: "pFT", color: colors.ft },
      { key: "pNO", color: colors.no }
    ];
    ...
    svg.append("text")
      ...
      .attr("transform", `rotate(90 ${w - 10} ${(margin.top + h - margin.bottom)/2})`)
      .attr("font-size", 12)
      .attr("fill", COLORS.muted)
      .text("Días de demora");

    // línea de demoras (arriba de todo)
    const line = d3.line()
      .defined(d => Number.isFinite(d.avgD))
      .x(d => x(d.m) + x.bandwidth()/2)
      .y(d => y1(d.avgD));
    ...
    // puntos para visibilidad
    svg.append("g")
      .selectAll("circle")
      .data(dataMes.filter(d => Number.isFinite(d.avgD)))
      .enter()
      .append("circle")
      .attr("cx", d => x(d.m) + x.bandwidth()/2)
      .attr("cy", d => y1(d.avgD))
      .attr("r", 4.5)
  ...
  // Fallback Chart.js (si D3 no está)
  // (dejamos tu implementación previa)
  const agg = new Map();
  const monthsSet = new Set();
  ...
  chartMes = new Chart(canvas.getContext("2d"), {
    ...
    options: {
      ...
      scales: {
        x: { stacked: true, ticks:{ color: COLORS.muted } },
        y: { stacked: true, min:0, max:100, ticks:{ color: COLORS.muted, callback:(v)=> v + "%" } }
      },
      plugins: {
