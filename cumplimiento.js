...
function detectDelimiter(csvText) {
  ...
  // crude counts; good enough for our files
    ...
    options: {
      ...
      plugins: {
        ...
        tooltip: {
          callbacks: {
            label: (c) => {
              // Línea (eje derecho): días de demora
              if (c.dataset?.yAxisID === "y1" || c.dataset?.type === "line") {
                const v = (c.parsed.y ?? 0);
                return ` ${c.dataset.label}: ${Number(v).toFixed(1).replace(".", ",")} días`;
              }
              // Barras apiladas: cantidad + %
...
window.addEventListener("DOMContentLoaded", () => {
  ...
  fetch(csvUrl)
    ...
    .then(text => {
      ...
      const m = parseDelimited(text, autoDelim);
      const usedDelim = autoDelim;
      // Si quedó una sola columna, probamos el otro delimitador
      if (m[0] && m[0].length === 1) {
        const other = usedDelim === ';' ? ',' : ';';
        const m2 = parseDelimited(text, other);
        if (m2[0] && m2[0].length > 1) {
          m.length = 0; m.push(...m2);
        }
      }
      ...
      if (!CLIENT_COL) {
        showError("No encuentro columna CLIENTE. Revisá delimitador/encabezados. Probé: " + CLIENT_CANDIDATES.join(" / "));
        return;
      }
