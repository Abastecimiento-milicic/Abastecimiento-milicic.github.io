...
function buildChartMes(rows){
  ...
  if (!window.Plotly) {
    showError("No se cargó Plotly. Revisá el <script> de Plotly en index.html.");
    return;
  }

  const agg = new Map();
  for (const r of rows){
    const mk = monthKey(r[MONTH_COL]) || monthKeyFromDate(r[DATE_COL]);
    if (!mk) continue;
    ...
    if (Number.isFinite(dv)) { c.dsum += dv; c.dcnt += 1; } // incluye negativos
  }
  ...
  const tot = months.map((_,i)=> qAT[i]+qFT[i]+qNO[i]);

  const pAT = months.map((_,i)=> tot[i] ? (qAT[i]*100/tot[i]) : 0);
  const pFT = months.map((_,i)=> tot[i] ? (qFT[i]*100/tot[i]) : 0);
  const pNO = months.map((_,i)=> tot[i] ? (qNO[i]*100/tot[i]) : 0);
  ...
  const traceAT = {
    ...
    y: pAT,
    customdata: qAT,
    hovertemplate: "<b>%{x}</b><br>Entregados AT: %{customdata:,d} (%{y:.1f}%)<extra></extra>",
  ...
  const traceFT = {
    ...
    y: pFT,
    customdata: qFT,
    hovertemplate: "<b>%{x}</b><br>Entregados FT: %{customdata:,d} (%{y:.1f}%)<extra></extra>",
  ...
  const traceNO = {
    ...
    y: pNO,
    customdata: qNO,
    hovertemplate: "<b>%{x}</b><br>No entregados: %{customdata:,d} (%{y:.1f}%)<extra></extra>",
    marker: { color: COLORS.red }
  };
  const traceLine = {
    type: "scatter",
    mode: "lines+markers",
    name: "Promedio dias de demora",
    x: months,
    y: avgD,
    yaxis: "y2",
    hovertemplate: "<b>%{x}</b><br>Promedio dias de demora: %{y:.0f} días<extra></extra>",
  ...
  const layout = {
    barmode: "stack",
    bargap: 0.25,
    margin: { l: 55, r: 65, t: 10, b: 60 },
    xaxis: { fixedrange: true },
    yaxis: { range: [0, 100], ticksuffix: "%", fixedrange: true, gridcolor: "rgba(0,0,0,0.08)" },
    yaxis2: {
  ...
  Plotly.newPlot(el, [traceAT, traceFT, traceNO, traceLine], layout, config);
}
