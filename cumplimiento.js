...
function buildChartMes(rows) {
  // Prefer D3 if available (for better control of layering + dual axis).
  if (window.d3) {
    const agg = new Map();
    ...
    for (const r of rows) {
      ...
      if (!agg.has(mk)) agg.set(mk, { at: 0, ft: 0, no: 0, dsum: 0, dcnt: 0 });
      const c = agg.get(mk);
      ...
      // DIAS DE DEMORA (incluye negativos). Ignora vacíos.
      const raw = clean(r[DEMORA_COL]);
      if (raw !== "") {
        const dv = toNumber(raw);
    ...
    const avgD = months.map(m => {
      const o = agg.get(m);
      return o && o.dcnt ? (o.dsum / o.dcnt) : null;
    });
    ...
    // Clean and mount an SVG (reemplaza el canvas)
    container.innerHTML = "";
    const width = container.clientWidth || 900;
    const height = 320;
    const margin = { top: 12, right: 58, bottom: 40, left: 46 };
    ...
    const dVals = avgD.filter(v => v !== null);
    const dMin = d3.min(dVals);
    const dMax = d3.max(dVals);
    const y1 = d3.scaleLinear()
      .domain([dMin ?? 0, dMax ?? 1])
      .nice()
      .range([innerH, 0]);
    ...
    g.selectAll("path").attr("stroke", "transparent");

    // Axes
    g.append("g")
      .attr("transform", `translate(0,${innerH})`)
      .call(d3.axisBottom(x))
      .selectAll("text").attr("fill", COLORS.muted).style("font-weight", 800);
    ...
    g.append("g")
      ...
      .call(d3.axisRight(y1).ticks(6).tickFormat(d => Math.round(d)))
      .selectAll("text").attr("fill", COLORS.muted).style("font-weight", 800);

    // Stacked data
    const series = [
      { label: "Entregados AT", color: COLORS.green, p: pAT, q: qAT },
      { label: "Entregados FT", color: COLORS.amber, p: pFT, q: qFT },
      { label: "No entregados", color: COLORS.red, p: pNO, q: qNO }
    ];
    ...
    g.selectAll("text.lbl")
      ...
      .text(d => {
        if (!d.qty) return "";
        if (d.pct < 7) return "";
        return `${fmtInt(d.qty)} (${Math.round(d.pct)}%)`;
      });

    // Line on top (avg days)
    const pts = months.map((m, i) => ({ month: m, val: avgD[i] })).filter(p => p.val !== null);
    ...
    const items = [
      { label: "Entregados AT", color: COLORS.green, kind: "rect" },
      { label: "Entregados FT", color: COLORS.amber, kind: "rect" },
      { label: "No entregados", color: COLORS.red, kind: "rect" },
      { label: "Promedio dias de demora", color: COLORS.blue, kind: "line" }
    ];

    let lx = 0;
    items.forEach(it => {
      const gi = legend.append("g").attr("transform", `translate(${lx},0)`);
      if (it.kind === "line") {
        gi.append("line").attr("x1",0).attr("x2",26).attr("y1",-6).attr("y2",-6)
          .attr("stroke", it.color).attr("stroke-width", 3);
      } else {
        gi.append("rect").attr("x",0).attr("y",-12).attr("width",26).attr("height",10).attr("fill", it.color);
      }
      gi.append("text").attr("x", 34).attr("y",-4).attr("fill", COLORS.text)
        .style("font-weight", 900).style("font-size","12px").text(it.label);

      lx += (it.label.length * 7) + 70;
    });

    return;
  }

  // Fallback to Chart.js (si no cargó D3)
  try {
    const agg = new Map();
    const monthsSet = new Set();
    ...
    for (const r of rows) {
      ...
      if (raw !== "") {
        const dv = toNumber(raw);
        if (Number.isFinite(dv)) { c.dsum += dv; c.dcnt += 1; }
      }
    }
    ...
    const qFT = months.map(m => agg.get(m)?.ft ?? 0);
    const qNO = months.map(m => agg.get(m)?.no ?? 0);
    const avgD = months.map(m => { const o=agg.get(m); return o && o.dcnt ? (o.dsum/o.dcnt) : null; });
    ...
    chartMes = new Chart(canvas.getContext("2d"), {
      ...
      data: {
        labels: months,
        datasets: [
          { label: "Entregados AT", data: pAT, _q: qAT, stack:"s", backgroundColor: COLORS.green, order: 1 },
          { label: "Entregados FT", data: pFT, _q: qFT, stack:"s", backgroundColor: COLORS.amber, order: 1 },
          { label: "No entregados", data: pNO, _q: qNO, stack:"s", backgroundColor: COLORS.red, order: 1 },
          { type: "line", label: "Promedio dias de demora", data: avgD, yAxisID: "y1",
            borderColor: COLORS.blue, backgroundColor: COLORS.blue,
            pointRadius: 4, pointHoverRadius: 6, borderWidth: 3, tension: 0.25,
            fill: false, spanGaps: true, order: 100, datalabels: { display: false }
          },
        ]
      },
      options: {
        ...
        scales: {
          ...
          y: { stacked:true, beginAtZero:true, max:100, grid:{ color: COLORS.grid }, ticks:{ color: COLORS.muted, callback:(v)=> v + "%" } },
          y1: { position:"right", beginAtZero:true, grid:{ drawOnChartArea:false }, title:{ display:true, text:"Dias de demora", color: COLORS.muted }, ticks:{ color: COLORS.muted, precision:0 } }
        }
      }
    });
  } catch(e) {
