    ...
    series: [
      {
        ...
        data: (() => {
          // Paleta SEGURA (Sin rojos, sin naranjas, sin rosas)
          const safePalette = [
            "#0d6efd", // Blue
            "#0dcaf0", // Cyan
            "#20c997", // Teal
          ...
          return items.map((it, i) => {
            ...
            // Indice estable basado en el nombre para consistencia entre filtros
            const idx = stableNames.indexOf(it.name);
            let finalColor = safePalette[(idx >= 0 ? idx : i) % safePalette.length];

            // Lógica ESTRICTA: Solo el MAX es ROJO.
            if (isMax) {
              finalColor = "#dc3545"; // Rojo Intenso (#dc3545)
            }
