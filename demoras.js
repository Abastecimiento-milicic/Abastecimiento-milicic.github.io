...
function buildTabla() {
  ...
  for (const m of months) {
    ...
    // almacén (si hay más de uno, muestra "Varios")
    let alm = "-";
    if (ALMACEN_COL) {
      const vals = [...new Set(rowsM.map(r => clean(r[ALMACEN_COL])).filter(Boolean))];
      alm = vals.length === 1 ? vals[0] : (vals.length > 1 ? "Varios" : "-");
    } else if (CLIENT_COL) {
      // fallback si el CSV usa ALMACEN como CLIENT_COL
      const vals = [...new Set(rowsM.map(r => clean(r[CLIENT_COL])).filter(Boolean))];
      alm = vals.length === 1 ? vals[0] : (vals.length > 1 ? "Varios" : "-");
    }

    // sumas por área
    const sumByArea = {};
    for (const a of AREA_COLS) sumByArea[a] = 0;

    for (const r of rowsM) {
      for (const a of AREA_COLS) {
        sumByArea[a] += toNumber(r[a]);
      }
    }
    ...
    const tds = [
      ...
      `<td>${alm}</td>`,
      ...AREA_COLS.map(a => {
        const v = sumByArea[a] || 0;
        const heat = heatColor(a, v); // tu función de mapa de calor
        return `<td style="background:${heat.bg};color:${heat.fg}">${fmtInt(v)}</td>`;
      })
    ];
