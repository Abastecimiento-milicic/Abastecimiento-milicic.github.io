...
function isTruthyAreaValue(v, areaName = null) {
    const t = clean(v);
    if (!t) return false;
    if (t === "0") return false;

    // Verificar si debemos excluir COMPRAS EQUIPOS
    if (areaName && norm(areaName) === norm("COMPRAS EQUIPOS")) {
        const check = document.getElementById("checkEquipos");
...
function filteredRows() {
    ...
    // FILTRO "COMPRAS EQUIPOS": Si está destildado, ignoramos cualquier fila que tenga esa marca
    // Esto asegura coincidencia con la clasificación excluyente de Cumplimiento
    const checkEquipos = document.getElementById("checkEquipos");
    if (checkEquipos && !checkEquipos.checked) {
        // Buscamos la columna de COMPRAS EQUIPOS entre las detectadas
        const colEquipos = AREA_COLS.find(a => norm(a) === norm("COMPRAS EQUIPOS"));
        if (colEquipos) {
            rows = rows.filter(r => !isTruthyAreaValue(r[colEquipos], colEquipos));
        }
    }
...
function countDemoras(rows) {
    ...
    // Solo contamos las filas que tengan al menos UN área con demora activa
    const checkEquipos = document.getElementById("checkEquipos");
    const includeEquipos = checkEquipos ? checkEquipos.checked : true;

    return rows.filter(r => {
        return AREA_COLS.some(a => {
            if (!includeEquipos && norm(a) === norm("COMPRAS EQUIPOS")) return false;
            return isTruthyAreaValue(r[a], a);
        });
    }).length;
}
...
function aggByMonth(rows) {
    ...
    for (const r of rows) {
        ...
        // Solo contamos si el mes tiene alguna demora acta en las áreas visibles
        const hasActiveDemora = AREA_COLS.some(a => {
            if (!includeEquipos && norm(a) === norm("COMPRAS EQUIPOS")) return false;
            return isTruthyAreaValue(r[a], a);
        });
...
function aggAreas(rows) {
    ...
    for (const a of AREA_COLS) {
        if (!includeEquipos && norm(a) === norm("COMPRAS EQUIPOS")) continue;
        out.set(a, 0);
    }

    for (const r of rows) {
        for (const [a, currentVal] of out.entries()) {
            if (isTruthyAreaValue(r[a], a)) out.set(a, currentVal + 1);
        }
    }
    return out;
}
...
function buildChartMes() {
    ...
    // Y también COMPRAS EQUIPOS si el checkbox está destildado
    const checkEquipos = document.getElementById("checkEquipos");
    const includeEquipos = checkEquipos ? checkEquipos.checked : true;
    ...
    const visibleAreas = AREA_COLS.filter(area => {
        ...
        if (!includeEquipos && na === norm("COMPRAS EQUIPOS")) return false;
        return true;
    });
...
function buildTabla() {
    ...
    // header - Excluir EQUIPOS MENORES y COMPRAS EQUIPOS (si aplica)
    const checkEquipos = document.getElementById("checkEquipos");
    const includeEquipos = checkEquipos ? checkEquipos.checked : true;
...
window.addEventListener("DOMContentLoaded", () => {
    ...
    fetch(csvUrl)
        ...
        .then(text => {
            ...
            document.getElementById("checkEquipos")?.addEventListener("change", () => {
                applyAll();
            });
        })
        .catch(err => {
