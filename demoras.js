...
function buildTabla() {
  ...
  for (const m of months) {
    ...
    const tds = [
      ...
      ...AREA_COLS.map(a => { const v = areaMap.get(a) || 0; return `<td class="td-num" data-v="${v}">${fmtInt(v)}</td>`; })
    ];
...
/* ===== Heatmap por fila (blanco -> naranja -> rojo) ===== */
function lerp(a,b,t){ return Math.round(a + (b-a)*t); }

function color3(t){
  // 0: blanco, 0.5: naranja, 1: rojo
  const white = [255,255,255];
  const orange = [255,165,0];
  const red = [220,53,69];
  if (t <= 0.5){
    const tt = t / 0.5;
    return `rgb(${lerp(white[0],orange[0],tt)},${lerp(white[1],orange[1],tt)},${lerp(white[2],orange[2],tt)})`;
  } else {
    const tt = (t-0.5) / 0.5;
    return `rgb(${lerp(orange[0],red[0],tt)},${lerp(orange[1],red[1],tt)},${lerp(orange[2],red[2],tt)})`;
  }
}
...
function applyHeatmapPorFila(){
  ...
  trs.forEach(tr => {
    ...
    const vals = cells.map(td => Number(td.dataset.v ?? td.textContent.replace(/\./g,'').replace(',','.') || 0));
